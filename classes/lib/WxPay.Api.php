...
class WxPayApi
{
    ...
    public static function unifiedOrder($inputObj, $timeOut = 6)
    {
        ...
        if (!$inputObj->IsNotify_urlSet()) {
            $inputObj->SetNotify_url(Configuration::get('WEIXIN_NOTIFY_URL'));//异步通知url
        }

        $inputObj->SetAppid(Configuration::get('WEIXIN_APPID'));//公众账号ID
        $inputObj->SetMch_id(Configuration::get('WEIXIN_MCH_ID'));//商户号
